...
class FinVizQueryParams(QueryParams):
    """
    Query Parameters for FinnViz queries
...
class FivizLEAPSFetcher(
    Fetcher[
        FinvizQueryParams,
        List[FinvizData],
    ]
):
    """ Finviz LEAPS Fetcher.
    ...
    @staticmethod
    def transform_data(
        query: FinVizDataQueryParams, data: List[dict], **kwargs: Any
    ) -> List[FinVizDataData]:
        ...
        """
        return [FinVizDataData(**d) for d in data]
